...
namespace TeacherAssistant.Pages.LessonForm
{
    ...
    {
        private readonly ModuleActivation<LessonFormToken> _activation;
        ...
        public LessonFormModel(ModuleActivation<LessonFormToken> activation, LocalDbContext db)
        {
            ...
            this.WhenActivated(c =>
            {
                this.WhenAnyValue(model => model.SelectedStream)
                    ...
                    (
                        ...
                        {
                            this.Groups.Clear();
                            this.Groups.AddRange(stream.Groups?.ToList() ?? new List<GroupEntity>(0));
                ...
                this.WhenAnyValue(model => model.Lesson)
                    ...
                    (
                        lesson =>
                        {
                            this.SelectedStream = lesson.Stream ?? this.Streams.First();
                            this.Description = lesson.Description ?? "";
                            this.LessonDate = lesson.Date ?? DateTime.Today;
                            var lessonType = this.LessonTypes.Find(view => view.Type == lesson.LessonType);
                            this.SelectedLessonType = lessonType ?? this.LessonTypes.First();
                            this.SelectedSchedule = lesson.Schedule ?? this.ScheduleList.First();
                        }
                    ).DisposeWith(c);
        ...
        [Reactive]
        [NotNull]
        public StreamEntity SelectedStream { get; set; }
        ...
        [Reactive]
        public List<LessonTypeView> LessonTypes { get; set; }
        ...
        [Reactive]
        [NotNull]
        public LessonTypeView SelectedLessonType { get; set; }

        [Reactive]
        [NotNull]
        public ScheduleEntity SelectedSchedule { get; set; }

        [Reactive]
        public string Description { get; set; }

        [Reactive]
        public bool SetAllPresent { get; set; }
        ...
        {
            await Save();
            await _activation.Token.ComponentHost.AddPageAsync(new RegistrationPageToken("Регистрация", _originalEntity));
