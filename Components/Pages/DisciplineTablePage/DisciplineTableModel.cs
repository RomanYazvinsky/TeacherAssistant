...
namespace TeacherAssistant.Pages.DisciplineTablePage
{
    public class DisciplineTableModel : AbstractModel<DisciplineTableModel>
    {
        ...
        public DisciplineTableModel(LocalDbContext db, WindowComponentHost host)
        {
            _db = db;
            _host = host;
            ...
            Observable.Merge(WhenAdded<DisciplineEntity>(), WhenRemoved<DisciplineEntity>(),
                    WhenUpdated<DisciplineEntity>())
        ...
        {
            _host.AddPageAsync(new DisciplineFormToken(this.SelectedDiscipline.Name, this.SelectedDiscipline));
        }
